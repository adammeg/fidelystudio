...
const influencerCampaignSchema = z.object({
  influencerName: z.string().trim().min(2).max(100),
...
export async function studioMutate(
  ...
  const protectedWrite = clean.startsWith("customers/") || ["campaigns", "influencer-campaigns", "influencers", "referral", "loyalty", "widgets", "whatsapp"].some((prefix) => clean === prefix || clean.startsWith(`${prefix}/`));
  if (protectedWrite) {
  ...
  if (clean === "influencer-campaigns" && method === "POST") {
  ...
  if (clean === "account" && method === "DELETE") {
    ...
    await Promise.all([
      ...
      StudioCustomer.deleteMany({ user: userId }), StudioInfluencer.deleteMany({ user: userId }), StudioInfluencerCampaign.deleteMany({ user: userId }),
